...
In the third case, the planned trajectories were replayed under simulated time-varying communication delays over $30$ trials. To evaluate our delay-robust CLIK controller, we re-implemented the standard AnyTeleop \cite{anyteleop} baseline under the identical simulated latency of $h_m = 32\,\text{ms}$. As shown in Table~\ref{tab:tracking_errors}, AnyTeleop suffered from significant control drift and joint oscillations due to lack of delay compensation, whereas our proposed CLIK controller maintained stable tracking.

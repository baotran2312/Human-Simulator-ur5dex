...
\begin{enumerate}
    ...
    \item \textbf{Action Space} $\mathcal{A}$: The action $\mathbf{a}_t \in \mathcal{A}$ corresponds to the continuous stiffness gain offsets:
    \begin{equation}
    \mathbf{a}_t = \Delta \mathbf{K}_{\theta}(t) \in [\Delta \mathbf{K}_{\text{min}}, \Delta \mathbf{K}_{\text{max}}]
    \end{equation}
    which dynamically scales the diagonal entries of the virtual stiffness matrix $\mathbf{K}_{\theta}(t) = \mathbf{K}_{\theta, 0} + \text{diag}(\mathbf{a}_t)$.
...
The compliance policy is trained offline in Isaac Sim using massive parallel rollouts via DexGraspNet templates. To ensure robustness to real-world communication delays $h(t)$ estimated via the LKF framework, we apply domain randomization to the observation latencies during training, matching the maximum allowable delay bound $h_m$ calculated via the LMI constraints. This ensures the learned policy remains stable under network jitter when deployed online.
